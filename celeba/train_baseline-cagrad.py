...
args.output_dir = "outputs/baselines-cagrad/" + str(args).replace(", ", "/").replace(
    "'", ""
...
def train_multi_task():
    ...
    train_loader, train_dst, val_loader, val_dst = datasets.get_dataset(
        params, configs, batch_size=args.batch_size
    )
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch in tqdm(train_loader, total=len(train_loader)):
            ...
            grad = CAGrad(torch.stack([grads[t] for t in tasks]), alpha=args.c)
            ...
            for name, param in model["rep"].named_parameters():
                if param.grad is not None:
                    ...
                    param.grad = (
                        grad[shared_idx : shared_idx + length]
                        .data.clone()
