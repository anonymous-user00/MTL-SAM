...
args.output_dir = "outputs/baselines-pcgrad/" + str(args).replace(", ", "/").replace(
    "'", ""
...
def train_multi_task():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch in tqdm(train_loader, total=len(train_loader)):
            ...
            grad = PCGrad([grads[t] for t in tasks])

            # Scaled back-propagation
            for idx, (name, param) in enumerate(model["rep"].named_parameters()):
                if param.grad is not None:
                    param.grad = grad[idx].data.clone()
            optimizer.step()
            optimizer.zero_grad()
