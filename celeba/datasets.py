...
def get_dataset(params, configs, batch_size=256, num_workers=2):
    ...
    if "mnist" in params["dataset"]:
        ...
        val_loader = torch.utils.data.DataLoader(
            val_dst, batch_size=100, shuffle=True, num_workers=num_workers
        )
    ...
    if "cityscapes" in params["dataset"]:
        train_dst = CITYSCAPES(
            ...
            split=["train"],
            img_size=(
                configs["cityscapes"]["img_rows"],
                configs["cityscapes"]["img_cols"],
            ),
